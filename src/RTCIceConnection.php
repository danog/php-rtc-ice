...
class RTCIceConnection extends EventEmitter implements RTCIceConnectionInterface, ReceiverInterface
{
    ...
    private function advanceBindingCheck(): void
    {
        ...
        // tryCheckBinding leaves isBindingWait set when a check is already in
        // flight (early checks start one before bindCheck runs). Failing here
        // would abort ICE while that transaction is still outstanding.
        if ($this->isBindingWait) {
            return;
        }
    ...
    private function tryCheckBinding(): bool
    {
        ...
        // Nothing new was started. If a check is already in flight (typical for
        // early Binding requests processed just before bindCheck), wait for it.
